...
(function(){
let filter_group = document.querySelector('select[name="filter_group"]');
if(filter_group){
    filter_group.addEventListener('change',(e)=>{
        let path = window.location.pathname.split('/')
        let linkRedirect = '/'+ path[1]+'/'+path[2]+'/filter_group/'+e.target.value;
...
let group_id = document.querySelector('select[name="group_id"]');
let group_name = document.querySelector('#group_name'); 
if(group_id){
    group_id.addEventListener('change',(e)=>{
        group_name.value =  e.target.options[e.target.options.selectedIndex].text;
        });
};
...
let input_slug = document.querySelector('input[name="slug"]'); 
let input_name = document.querySelector('input[name="name"]'); 
if(input_name) {
    input_name.addEventListener('keyup', (e) => {
        output_name =  removeAccents(e.target.value);
        if(input_slug){
            input_slug.value =  output_name
        }
     })
};
let form = document.querySelector('form[name="form-upload"]'); 

if(form){
    form.addEventListener('submit', () =>{
    let avatar = document.querySelector('input[name="avatar"]');
    document.querySelector('input[name="avatar"]').remove(); 
    form.appendChild(avatar).style.display = 'none'; 
    })
}
